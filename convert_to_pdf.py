...
Converteert een Markdown-bestand naar een nette PDF.
...
hr  {{ border: none; border-top: 1px solid #e0e0e0;
       margin: 20pt 0; page-break-before: always; }}
...
table {{
    ...
    margin: 8pt 0; font-size: 9.5pt; page-break-inside: avoid;
}}
...
<body>{body}</body></html>"""
    ...
    print("  [pdf] Render PDF …")
    HTML(string=html, base_url=".").write_pdf(
        str(output_pdf),
        stylesheets=[CSS(string=CSS_TEMPLATE.format(title=title))],
    )
